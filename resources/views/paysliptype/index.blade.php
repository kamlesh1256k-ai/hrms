...
<div class="row">
        ...
        <div class="col-12">
            <ul class="nav nav-tabs mb-3" id="addressMasterTabs" role="tablist">
                <li class="nav-item"><button class="nav-link active" data-bs-toggle="tab" data-bs-target="#list-pane" type="button" role="tab">{{ __('Payslip Types') }}</button></li>
                                ...
                                <tbody>
                                    @foreach ($paysliptypes as $paysliptype)
                                        <tr>
                                            ...
                                            <td>{{ $masterData['countryNames'][$paysliptype->country] ?? ($paysliptype->country ?? '—') }}</td>
                                            <td>{{ $masterData['stateNames'][$paysliptype->state] ?? ($paysliptype->state ?? '—') }}</td>
                                            <td>{{ $masterData['cityNames'][$paysliptype->city] ?? ($paysliptype->city ?? '—') }}</td>
                                            <td class="Action">
