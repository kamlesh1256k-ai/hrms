...
<div class="modal-body">
    <div class="row">
        ...
        {{-- Source of hire & assigned recruiter --}}
        <div class="form-group col-md-6">
            {{ Form::label('source', __('Source of Hire'), ['class' => 'form-label']) }}
            <select name="source" class="form-control select2">
                <option value="">{{ __('-- Select source --') }}</option>
                @foreach(\App\Models\JobApplication::$sources as $key => $label)
                    <option value="{{ $key }}">{{ $label }}</option>
                @endforeach
            </select>
        </div>
        <div class="form-group col-md-6">
            {{ Form::label('source_detail', __('Source Detail (optional)'), ['class' => 'form-label']) }}
            {{ Form::text('source_detail', null, ['class' => 'form-control', 'placeholder' => __('e.g. Referred by Rahul / LinkedIn InMail')]) }}
        </div>
        <div class="form-group col-md-6">
            {{ Form::label('recruiter_id', __('Assigned Recruiter'), ['class' => 'form-label']) }}
            <select name="recruiter_id" class="form-control select2">
                <option value="">{{ __('-- Unassigned --') }}</option>
                @foreach(\App\Models\User::where('created_by', \Auth::user()->creatorId())->whereIn('type', ['hr','company','employee'])->orderBy('name')->get() as $rec)
                    <option value="{{ $rec->id }}">{{ $rec->name }} <small>({{ $rec->type }})</small></option>
                @endforeach
            </select>
        </div>
        ...
        <div class="form-group col-md-12 letter d-none">
            ...
            {{ Form::textarea('cover_letter', null, ['class' => 'form-control', 'rows' => '3', 'placeholder' => __('Enter Cover Letter')]) }}
        </div>
