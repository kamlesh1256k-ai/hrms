        ...
        @forelse($employees as $employee)
            <div class="col-xl-3">
                <div class="card  text-center">
                    ...
                    <div class="card-body">
                        @php
                            // Prefer the employee's uploaded "Passport size Photo" document as
                            // the profile picture. Falls back to the linked user's avatar,
                            // then to the default avatar.png.
                            $passportDoc = \App\Models\EmployeeDocument::where('employee_id', $employee->employee_id)
                            ...
                            if (!empty($passportDoc)) {
                                $profilePhotoUrl = rtrim(\App\Models\Utility::get_file('uploads/document/'), '/') . '/' . $passportDoc;
                            } elseif (!empty($employee->user->avatar)) {
                                $profilePhotoUrl = asset(Storage::url('uploads/avatar')) . '/' . $employee->user->avatar;
                            } else {
                                $profilePhotoUrl = asset(Storage::url('uploads/avatar')) . '/avatar.png';
                            }
                        @endphp
                        <div class="avatar">
                            <a href="{{ $profilePhotoUrl }}" target="_blank">
