...
<div class="modal-body">
    ...
    <div class="row">
            ...
            <div class="form-group">
                ...
                <div class="form-icon-user">
                    <div class="department_div">
                        <select class="form-control department_id" name="department_id[]" id="dept-select"
        ...
        <div class="col-lg-12 col-md-12 col-sm-12">
            <div class="form-group">
                {{ Form::label('meet_link', __('Meeting Link'), ['class' => 'form-label']) }}
                <div class="input-group">
                    <span class="input-group-text"><i class="ti ti-video"></i></span>
                    {{ Form::url('meet_link', null, ['class' => 'form-control', 'placeholder' => __('Paste Google Meet / Zoom link here (optional)')]) }}
                </div>
                <small class="text-muted">{{ __('e.g. https://meet.google.com/xxx-xxxx-xxx') }}</small>
            </div>
        </div>
...
<script>
    var getDeptUrl = "{{ route('meeting.getdepartment') }}";
    var getEmpUrl = "{{ route('meeting.getemployee') }}";
    var csrfToken = "{{ csrf_token() }}";
    var allEmpLabel = "{{ __('All Employees') }}";
    ...
    function getDepartment(bid) {
        $.ajax({
            ...
            success: function(data) {
                var html = '<select class="form-control department_id" name="department_id[]" id="dept-select" multiple required>';
                ...
                var allDeptIds = Object.keys(data);
                if (allDeptIds.length > 0) {
                    getEmployee(allDeptIds);
                }
            }
        });
    }
    ...
    function getEmployee(did) {
        $.ajax({
            ...
            success: function(data) {
                var html = '<select class="form-control employee_id" name="employee_id[]" id="emp-select" multiple>';
                html += '<option value="0">' + allEmpLabel + '</option>';
                $.each(data, function(key, value) {
                    html += '<option value="' + key + '">' + value + '</option>';
                });
                html += '</select>';
