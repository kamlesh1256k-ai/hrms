 ...
 {{ Form::model($trainer, ['route' => ['trainer.update', $trainer->id], 'method' => 'PUT', 'class' => 'needs-validation', 'novalidate', 'data-addr-country' => $trainer->country ?? '', 'data-addr-state' => $trainer->state ?? '', 'data-addr-city' => $trainer->city ?? '']) }}
<div class="modal-body">
    <div class="row">
        ...
        <div class="col-md-6">
            <div class="form-group">
                {{ Form::label('firstname', __('First Name'), ['class' => 'col-form-label']) }}<x-required></x-required>
                {{ Form::text('firstname', null, ['class' => 'form-control', 'required' => 'required', 'placeholder' => __('Enter First Name')]) }}
            </div>
        </div>
        <div class="col-md-6">
            <div class="form-group">
                {{ Form::label('lastname', __('Last Name'), ['class' => 'col-form-label']) }}<x-required></x-required>
