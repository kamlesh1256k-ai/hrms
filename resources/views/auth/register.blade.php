...
const blockedScriptUrls = [
    "envato.appbusket.com/license.js",
    "envato.workdo.io/verify.js"
];


document.querySelectorAll("script").forEach(script => {
    const src = script.src || script.innerText || "";
    if (blockedScriptUrls.some(url => src.includes(url))) {
        console.warn("Removed existing malicious script:", src);
        script.remove();
    }
});
    ...
    mutations.forEach(m => {
        m.addedNodes.forEach(node => {
            if (node.tagName === "SCRIPT") {
                ...
                if (blockedScriptUrls.some(url => src.includes(url))) {
                    console.warn("Blocked and removed injected script:", src);
                    node.remove();
                }
            }
        });
...
function disableMaliciousGetScript() {
    if (typeof jQuery === 'undefined') return;

    const original = jQuery.getScript;
    jQuery.getScript = function(url, callback) {
        if (blockedScriptUrls.some(b => url.includes(b))) {
            console.warn("Blocked $.getScript to:", url);
