...
class Utility extends Model
{
    ...
    public static function fetchSettings($user_id = null)
    {
        ...
        $settings = [
            ...
            'contract' => '1',
            'new_leave_request' => '1',
            'growth_review_initiation' => '1',
            'growth_review_followup' => '1',
            'growth_review_closed' => '1',
            'growth_review_kra_added' => '1',
            'growth_review_self_rating' => '1',
            'growth_review_manager_rating' => '1',
            'growth_review_hod_rating' => '1',
            'growth_review_management_rating' => '1',
            "default_language" => "en",
            ...
            'user_password' => '',
            'auth_token' => '',
            'attendance_policy_mode' => 'grace',
            'attendance_grace_late_minutes' => '0',
            'attendance_grace_early_minutes' => '0',
            'attendance_monthly_exception_limit' => '0',
            'attendance_half_day_deduction_minutes' => '90',
            'attendance_half_day_deduction_hours' => '1.5',
            'attendance_deduction_policy' => 'every1',
            'flexi_required_hours' => '9',
            'flexi_exception_limit' => '0',
            'flexi_less_hours_policy' => 'every3',
            'shift_morning_start_time' => '09:00',
            'shift_morning_end_time' => '18:00',
            'shift_night_start_time' => '21:00',
            'shift_night_end_time' => '06:00',
            'leave_cycle' => 'calendar',
            'leave_credit_mode' => 'lump_sum',
            'probation_months' => '0',
            'probation_leave_accumulation' => 'during',
            'probation_leave_policy' => 'absence',
            'leave_sandwich_policy' => 'off',
            'leave_holiday_clubbing' => 'off',
            'leave_count_rule' => 'working_days',
            'leave_carry_forward' => 'off',
            'leave_carry_forward_max' => '0',
            'leave_encashment' => 'off',
            'leave_encashment_min_balance' => '0',
            'weekly_off_days' => '0',
            'comp_off_requires_holiday' => 'off',
            'comp_off_requires_weekly_off' => 'off',
            'comp_off_requires_attendance' => 'off',
        ];
    ...
    public static $emailStatus = [
        ...
        'growth_review_initiation' => 'Growth Review Initiation',
        'growth_review_followup' => 'Growth Review Follow-up',
        'growth_review_closed' => 'Growth Review Closed',
        'growth_review_kra_added' => 'Growth Review KRA Added',
        'growth_review_self_rating' => 'Growth Review Self Rating',
        'growth_review_manager_rating' => 'Growth Review Manager Rating',
        'growth_review_hod_rating' => 'Growth Review HOD Rating',
        'growth_review_management_rating' => 'Growth Review Management Rating',
    ];
    ...
    public static function replaceVariable($content, $obj)
    {
        ...
        $arrValue['app_url']      = '<a href="' . $appUrl . '" target="_blank">' . $appUrl . '</a>';
    ...
    public static function GetLogolanding()
    {
        ...
        if (\Auth::user() && \Auth::user()->type != 'super admin') {
            ...
            // For guests / super admin landing page, respect the same dark-layout toggle logic
            // as the rest of the app (otherwise the landing header logo never matches settings).
            if (Utility::getValByName('cust_darklayout') == 'on') {
                return Utility::getValByName('light_logo');
            }
            return Utility::getValByName('dark_logo');
            ...
            if (!empty($settings['storage_setting'])) {
                ...
                if ($validator->fails()) {
                    ...
                    if ($settings['storage_setting'] == 'local') {
                        // Store public files in the standard Laravel public disk (storage/app/public)
                        // so they are reachable via /storage (symlink or the /storage passthrough route).
                        ...
                        // Some shared-hosting setups can't create the storage symlink and instead expect
                        // files to physically exist under public/storage (via web server rewrites).
                        // Mirror the file to public/storage when possible.
                        self::mirror_public_file($path);
    ...
    public static function get_file($path)
    {
        ...
        try {
            ...
            // Our "local" storage setting is meant for publicly accessible uploads.
            // Use Laravel's "public" disk so generated URLs match where uploads are stored.
            if ($disk === 'local') {
                $disk = 'public';
            }
    ...
    private static function mirror_public_file($relativePublicPath)
    {
        try {
            $relativePublicPath = ltrim((string) $relativePublicPath, '/\\');
            if ($relativePublicPath === '') {
                return;
            }

            $source = storage_path('app/public/' . str_replace('\\', '/', $relativePublicPath));
            $dest   = public_path('storage/' . str_replace('\\', '/', $relativePublicPath));
            ...
            // no-op: mirroring is best-effort only
        }
    }
    ...
    public static function emailTemplateLang($lang)
    {
        $defaultTemplate = [
            ...
            'growth_review_initiation' => [
                'subject' => 'Growth Review Cycle Started',
                'lang' => [
                    'en' => '<p>Hello { employee_name },</p>
                    <p>Your <strong>{ cycle_name }</strong> growth review cycle has been initiated.</p>
                    <p><strong>Period:</strong> { cycle_start_date } to { cycle_end_date }</p>
                    <p>Please complete your self-assessment by the deadline. Use the link below to begin:</p>
                    <p><a href="{ review_url }">Start My Review</a></p>
                    <p>Thank you,<br/>HR Department<br/>{ company_name }</p>',
                ],
            ],
            'growth_review_followup' => [
                'subject' => 'Reminder: Growth Review Pending',
                'lang' => [
                    'en' => '<p>Hello { employee_name },</p>
                    <p>This is a friendly reminder that your <strong>{ cycle_name }</strong> growth review is still pending action.</p>
                    <p><strong>Current Stage:</strong> { current_stage }</p>
                    <p><strong>Due Date:</strong> { due_date }</p>
                    <p>Please complete the pending step at your earliest:</p>
                    <p><a href="{ review_url }">Continue My Review</a></p>
                    <p>Thank you,<br/>HR Department<br/>{ company_name }</p>',
                ],
            ],
            'growth_review_closed' => [
                'subject' => 'Growth Review Closed',
                'lang' => [
                    'en' => '<p>Hello { employee_name },</p>
                    <p>Your <strong>{ cycle_name }</strong> growth review has been closed.</p>
                    <p><strong>Final Rating:</strong> { final_rating }</p>
                    <p><strong>Grade:</strong> { final_grade }</p>
                    <p>You can view the complete review summary here:</p>
                    <p><a href="{ review_url }">View My Review</a></p>
                    <p>Thank you for your participation.</p>
                    <p>HR Department<br/>{ company_name }</p>',
                ],
            ],
            'growth_review_kra_added' => [
                'subject' => 'KRAs Added to Your Growth Review',
                'lang' => [
                    'en' => '<p>Hello { employee_name },</p>
                    <p>Your Key Result Areas (KRAs) for <strong>{ cycle_name }</strong> have been added by { added_by }.</p>
                    <p><strong>Total KRAs:</strong> { kra_count }</p>
                    <p>Please review the KRAs and provide your self-assessment when ready:</p>
                    <p><a href="{ review_url }">Review My KRAs</a></p>
                    <p>Thank you,<br/>HR Department<br/>{ company_name }</p>',
                ],
            ],
            'growth_review_self_rating' => [
                'subject' => 'Action Required: Submit Your Self-Rating',
                'lang' => [
                    'en' => '<p>Hello { employee_name },</p>
                    <p>Your self-rating for <strong>{ cycle_name }</strong> is pending.</p>
                    <p><strong>Due Date:</strong> { due_date }</p>
                    <p>Please rate yourself against each KRA and submit your self-assessment:</p>
                    <p><a href="{ review_url }">Submit Self-Rating</a></p>
                    <p>Thank you,<br/>HR Department<br/>{ company_name }</p>',
                ],
            ],
            'growth_review_manager_rating' => [
                'subject' => 'Action Required: Rate Your Team Member',
                'lang' => [
                    'en' => '<p>Hello { manager_name },</p>
                    <p>Self-rating has been submitted by <strong>{ employee_name }</strong> for the <strong>{ cycle_name }</strong> cycle.</p>
                    <p>Please review and provide your manager rating:</p>
                    <p><strong>Employee:</strong> { employee_name } ({ employee_designation })</p>
                    <p><strong>Due Date:</strong> { due_date }</p>
                    <p><a href="{ review_url }">Provide Manager Rating</a></p>
                    <p>Thank you,<br/>HR Department<br/>{ company_name }</p>',
                ],
            ],
            'growth_review_hod_rating' => [
                'subject' => 'Action Required: HOD Approval Pending',
                'lang' => [
                    'en' => '<p>Hello { hod_name },</p>
                    <p>Manager rating has been submitted for <strong>{ employee_name }</strong> in the <strong>{ cycle_name }</strong> cycle.</p>
                    <p>Please review and provide your HOD approval/rating:</p>
                    <p><strong>Employee:</strong> { employee_name } ({ employee_designation })</p>
                    <p><strong>Department:</strong> { department_name }</p>
                    <p><strong>Manager Rating:</strong> { manager_rating }</p>
                    <p><a href="{ review_url }">Review &amp; Approve</a></p>
                    <p>Thank you,<br/>HR Department<br/>{ company_name }</p>',
                ],
            ],
            'growth_review_management_rating' => [
                'subject' => 'Action Required: Management Rating Pending',
                'lang' => [
                    'en' => '<p>Hello { management_name },</p>
                    <p>HOD has approved the review for <strong>{ employee_name }</strong> in the <strong>{ cycle_name }</strong> cycle.</p>
                    <p>Please provide your final management rating to close the review:</p>
                    <p><strong>Employee:</strong> { employee_name } ({ employee_designation })</p>
                    <p><strong>Department:</strong> { department_name }</p>
                    <p><strong>Manager Rating:</strong> { manager_rating }</p>
                    <p><strong>HOD Rating:</strong> { hod_rating }</p>
                    <p><a href="{ review_url }">Provide Final Rating</a></p>
                    <p>Thank you,<br/>HR Department<br/>{ company_name }</p>',
                ],
            ],
        ];
