...
class SettingsController extends Controller
{
        ...
        if (\Auth::user()->type == 'company' || \Auth::user()->type == 'super admin') {
            if ($user->type == 'super admin') {
                ...
                if (\Schema::hasTable('shifts')) {
                    $shifts = Shift::where('created_by', \Auth::user()->creatorId())->orderBy('id', 'desc')->get();
                }
                ...
                return view('setting.company_settings', compact('settings', 'timezones', 'ips', 'EmailTemplates', 'currOfferletterLang', 'Offerletter', 'offerlang', 'Joiningletter', 'currjoiningletterLang', 'joininglang', 'experience_certificate', 'curr_exp_cetificate_Lang', 'explang', 'noc_certificate', 'currnocLang', 'noclang', 'webhooks', 'offerlangName', 'joininglangName', 'explangName', 'noclangName', 'shifts'));
            }
        } else {
    ...
    public function storeShiftSettings(Request $request)
    {
        if (\Auth::user()->type != 'company' && \Auth::user()->type != 'super admin') {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
        ...
        if (!\Schema::hasTable('shifts')) {
            return redirect()->back()->with('error', __('Shifts table not found. Please run migrations.'));
        }
        ...
        return redirect()->back()->with('success', __('Shift successfully created.'));
    }

    public function updateShiftSettings(Request $request, $id)
    {
        if (\Auth::user()->type != 'company' && \Auth::user()->type != 'super admin') {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
        ...
        $shift = Shift::where('created_by', \Auth::user()->creatorId())->findOrFail($id);
        $shift->update([
            'name' => $request->name,
            'start_time' => $request->start_time,
            'end_time' => $request->end_time,
        ...
        return redirect()->back()->with('success', __('Shift successfully updated.'));
    }

    public function destroyShiftSettings($id)
    {
        if (\Auth::user()->type != 'company' && \Auth::user()->type != 'super admin') {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
        ...
        return redirect()->back()->with('success', __('Shift successfully deleted.'));
    }
    ...
    public function store(Request $request)
    {
        if (\Auth::user()->type == 'company' || \Auth::user()->type == 'super admin') {
            if ($request->logo) {
                ...
                $request->validate(
                    [
                        'logo' => 'image|mimes:png,jpg,jpeg|max:20480',
                    ]
                );
            ...
            if ($request->logo_light) {
                $request->validate(['logo_light' => 'required|image|mimes:png,jpg,jpeg',]);
    ...
    public function saveCompanySettings(Request $request)
    {
        if (\Auth::user()->type == 'company' || \Auth::user()->type == 'super admin') {
            ...
            $user = \Auth::user();
            if ($request->has('company_name') || $request->has('company_address')) {
                $request->validate(
                    [
            ...
            if (!isset($request->leave_holiday_clubbing)) {
                $post['leave_holiday_clubbing'] = 'off';
            }
            if (!isset($request->leave_carry_forward)) {
                $post['leave_carry_forward'] = 'off';
            }
            if (!isset($request->leave_encashment)) {
                $post['leave_encashment'] = 'off';
            }
            if (!isset($request->comp_off_requires_holiday)) {
                $post['comp_off_requires_holiday'] = 'off';
            }
            if (!isset($request->comp_off_requires_weekly_off)) {
                $post['comp_off_requires_weekly_off'] = 'off';
            }
            if (!isset($request->comp_off_requires_attendance)) {
                $post['comp_off_requires_attendance'] = 'off';
            }

            if ($request->has('weekly_off_days')) {
                $post['weekly_off_days'] = implode(',', (array) $request->weekly_off_days);
            } elseif ($request->has('leave_cycle')) {
                $post['weekly_off_days'] = '';
            }
            unset($post['_token']);
    ...
    public function saveBusinessSettings(Request $request)
    {
        if (\Auth::user()->type == 'company' || \Auth::user()->type == 'super admin') {
            ...
            if ($request->company_logo_light) {
                ...
                $request->validate(
                    [
                        'company_logo_light' => 'image|mimes:png,jpg,jpeg|max:20480',
                    ]
                );
            ...
            if ($request->company_favicon) {
                $request->validate(
                    [
                        'company_favicon' => 'image|mimes:png,jpg,jpeg|max:20480',
                    ]
                );
    ...
    public function CookieConsent(Request $request)
    {
        ...
        if ($settings['enable_cookie'] == "on" && $settings['cookie_logging'] == "on") {
            ...
            $new_line = implode(',', [
                ...
                $query['country'] ?? '', $query['region'] ?? '', $query['regionName'] ?? '', $query['city'] ?? '', $query['zip'] ?? '', $query['lat'] ?? '', $query['lon'] ?? ''
            ]);
