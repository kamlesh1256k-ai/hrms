...
class InterviewScheduleController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (\Auth::user()->can('Create Interview Schedule')) {
            ...
            $schedule->round_type   = $request->round_type ?: 'technical';
            $schedule->round_label  = $request->round_label;
            $schedule->mode         = $request->mode ?: 'online';
    ...
    public function update(Request $request, LocalInterviewSchedule $interviewSchedule)
    {
        if (\Auth::user()->can('Edit Interview Schedule')) {
            ...
            $interviewSchedule->round_type     = $request->round_type ?: $interviewSchedule->round_type;
            $interviewSchedule->round_label    = $request->round_label;
            $interviewSchedule->mode           = $request->mode ?: $interviewSchedule->mode;
            $interviewSchedule->meeting_link   = $request->meeting_link;
    ...
    /**
     * POST /interview-schedule/{id}/feedback
     * Records the post-interview status, rating (1-5), feedback and recommendation.
     * Used by the interviewer to "complete" the round and decide selected/rejected/no_show.
     */
    public function recordFeedback(Request $request, LocalInterviewSchedule $interviewSchedule)
    {
        $data = $request->validate([
            'status'         => 'required|in:scheduled,completed,selected,rejected,no_show,cancelled',
            'rating'         => 'nullable|integer|min:1|max:5',
            'feedback'       => 'nullable|string|max:5000',
            'recommendation' => 'nullable|in:strong_yes,yes,maybe,no,strong_no',
        ]);

        $interviewSchedule->fill($data);
        $interviewSchedule->save();

        return redirect()->back()->with('success', __('Interview feedback saved.'));
    }
