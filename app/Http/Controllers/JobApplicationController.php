...
class JobApplicationController extends Controller
{
    ...
    public function store(Request $request)
    {
        if (\Auth::user()->can('Create Job Application')) {
            ...
            $job->source_detail   = $request->source_detail;
            $job->recruiter_id    = $request->recruiter_id ?: null;
    ...
    {
        $jobOnBoard       = JobOnBoard::where('created_by', \Auth::user()->creatorId())->findOrFail($id);
    ...
    public function jobBoardConvertData(Request $request, $id)
    {
        ...
        if (!empty($employee)) {
            ...
            $jobOnBoard->joining_date        = $request['company_doj'] ?? $jobOnBoard->joining_date;
            // Once employee is created, treat as "joined".
            $jobOnBoard->status              = 'confirm';
            $jobOnBoard->save();
        }
