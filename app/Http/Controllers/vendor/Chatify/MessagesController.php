...
class MessagesController extends Controller
{
    ...
    public function send(Request $request)
    {
        ...
        if (!$error->status) {
            ...
            // send to user using pusher (non-blocking)
            try {
                $pusherKey = trim((string) config('chatify.pusher.key'));
                $pusherSecret = trim((string) config('chatify.pusher.secret'));
                $pusherAppId = trim((string) config('chatify.pusher.app_id'));
                $pusherCluster = trim((string) data_get(config('chatify.pusher.options'), 'cluster', ''));
                ...
                Log::warning('Chatify push failed: ' . $e->getMessage());
            }
        }
    ...
    public function getContacts(Request $request)
    {
        try {
            ...
            Log::warning('Chatify getContacts query failed', ['message' => $e->getMessage()]);

            return Response::json([
                'contacts' => '',
                'total' => 0,
                'last_page' => 1,
                'allUsers' => '<p class="message-hint center-el"><span>'.__('Unable to load contacts').'</span></p>',
            ], 200);
        }
        ...
        // message unread data (header polls with type=custom)
    ...
    public function search(Request $request)
    {
        ...
        $records = User::where('id', '!=', Auth::user()->id)->where('name', '!=', 'Super Admin')
