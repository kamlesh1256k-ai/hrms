...
var messenger,
  ...
  previousUnseenTotal = 0,
  hasLoadedContactsOnce = false;
...
function playNotificationTone() {
  try {
    ...
    console.warn("Notification tone blocked", e);
  }
}

function playVoiceNotification(text = "New message received") {
  try {
...
function showTopMessageTooltip(text = "New message received") {
  const oldTooltip = document.getElementById("chatify-top-tooltip");
  if (oldTooltip) {
    oldTooltip.remove();
  }

  const tooltip = document.createElement("div");
  tooltip.id = "chatify-top-tooltip";
  ...
  tooltip.style.boxShadow = "0 10px 24px rgba(0,0,0,.25)";
...
function fetchMessages(id, type, newFetch = false) {
  ...
  if (messenger != 0 && !noMoreMessages && !messagesLoading) {
    ...
    $.ajax({
      ...
      dataType: "JSON",
      success: (data) => {
        const previousLastMessageId = lastFetchedMessageId;
        const incomingLastMessageId = data?.last_message_id ?? null;
        ...
        if (messagesPage == 1) {
          ...
          lastFetchedMessageId = incomingLastMessageId;
        } else {
...
function getContacts(refresh = false) {
  if (refresh) {
    contactsPage = 1;
    ...
    $.ajax({
      ...
      success: (data) => {
        ...
        if (contactsPage < 2) {
          ...
          const currentUnseenTotal = $(".listOfContacts")
            .find(".messenger-list-item b")
            .toArray()
            .reduce((sum, item) => {
              const value = parseInt($(item).text() || "0", 10);
              return sum + (isNaN(value) ? 0 : value);
            }, 0);

          if (hasLoadedContactsOnce && currentUnseenTotal > previousUnseenTotal) {
            playNotificationTone();
            playVoiceNotification("You have a new message");
            showTopMessageTooltip("You have a new message");
            if (typeof show_toastr === "function") {
              show_toastr("New Message", "You have a new message", "info");
            }
          }

          previousUnseenTotal = currentUnseenTotal;
          hasLoadedContactsOnce = true;

          const selectedContact = getMessengerId();
          const selectedExists =
            selectedContact &&
            selectedContact != 0 &&
            $(".listOfContacts")
              .find(`.messenger-list-item[data-contact=${selectedContact}]`).length > 0;

          const unreadFirstItem = $(".listOfContacts")
            .find(".messenger-list-item")
            .filter(function () {
          ...
          const unreadContactId = unreadFirstItem.attr("data-contact");
          if (unreadFirstItem.length && unreadContactId && unreadContactId != selectedContact) {
            unreadFirstItem.trigger("click");
          }
        } else {
          $(".listOfContacts").append(data.contacts);
        }
